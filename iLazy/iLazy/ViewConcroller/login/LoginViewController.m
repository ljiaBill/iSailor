...
-(void)button{
    ...
    self.loginBTN.backgroundColor = COLORNAVIGATION;
...
-(void)toSuccesslogin{
    ...
    if(self.isClick)
    {
        ...
        if([self.usernameText.text isEqualToString:@"" ]|| [self.passwordText.text isEqualToString:@""])
        {
            ...
            [dicn setObject:self.usernameText.text forKey:@"username"];
            [dicn setObject:[AESCrypt encrypt:self.passwordText.text password:USER_KEY] forKey:@"password"];
            
            Request *request = [[Request alloc]init];
            [request requestUserInfo:dicn andSuccess:^(NSDictionary *postDic) {
                NSLog(@"%@",postDic);
                ...
                if ([str isEqualToString:@"succeed"]) {
                    ...
                    if ([userstatus isEqual:@"0"]) {
                        ...
                        //将懒人登录的l_userid发送给 地图主页 ，接受l_userid
                        [MapView setValue:userstr forKey:@"l_userid"];
                        
                        NSLog(@"%@",[arr[0] objectForKey:@"userid"]);
...
//缩放    [self.usernamelabel.layer addAnimation:[self scale:[NSNumber numberWithFloat:1.0f] orgin:[NSNumber numberWithFloat:1.5f] durTimes:2.0f Rep:MAXFLOAT] forKey:nil];
